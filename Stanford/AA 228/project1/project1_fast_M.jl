...
function compute(infile, outfile)
    ...
    # ordering = [13, 12, 10, 3, 1, 9, 8, 4, 11, 6, 7, 2, 5]
    ...
    for k in 1:Max_itr
        ...
        update_M(r, G′, data, M, i_changed[1])
        y′ = is_cyclic(G′) ? -Inf : bayesian_score(n, r, G′, data, M, i_changed[2])
        if y′ > y
            y, dag = y′[end], G′
        else
            update_M(r, dag, data, M, i_changed[1])
            update_M(r, dag, data, M, i_changed[2])
        end
    end
